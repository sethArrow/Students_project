...
elseif(isset($_GET['orid_pg_id'])){$oridpage_id = $_GET['orid_pg_id'];$result = mysql_query("DELETE FROM orid_pages WHERE page_id = '$oridpage_id' ")or die ("Error in query: $result.".mysql_error());echo '<font size="-1" style="align: center;">Deletion Succesful.</font>';}elseif(isset($_GET['ugrf_id'])){$ugrf_call_id = $_GET['ugrf_id'];$result = mysql_query("DELETE FROM ugrf_calls WHERE ugrf_call_id = '$ugrf_call_id' ") or die ("Error in query: $result.".mysql_error());echo '<font size="-1" style="align: center;">Deletion Successful. </font>' ;}else{}
?>	
